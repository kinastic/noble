...
class Peripheral {
  ...
  async discoverServices(uuids) {
    if (this.gatt) {
      const serviceUuids = await this.gatt.discoverServices(uuids);
      for (const serviceUuid of serviceUuids) {
        const service = this.services.find(({ uuid }) => uuid === serviceUuid);
        if (service) {
          service.setGatt(this.gatt.gatt);
        } else {
          this.services.push(new Service(this.gatt.gatt, this.address, serviceUuid));
        }
      }
      return this.services;
    }
    return new Error("Gatt is NULL");
  }
  ...
  onDisconnect({ handle, reason }) {
    ...
    this.gatt = null;
    this.services.forEach(s => s.onDisconnect());
  }
