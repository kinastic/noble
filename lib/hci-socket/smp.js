...
class Smp {
  ...
  handlePairingRandom(data) {
    ...
    if (this._pcnf.toString('hex') === pcnf.toString('hex')) {
      ...
      // TODO test
      this.source.next({ event: 'stk', payload: { stk }});
      // this.emit('stk', stk);
    } else {
  ...
  handleEncryptInfo(data) {
    ...
    // TODO test
    this.source.next({ event: 'ltk', payload: { ltk } });
    // this.emit('ltk', ltk);
  }
  ...
  handleMasterIdent(data) {
    ...
    // TODO test
    this.source.next({ event: 'masterIdent', payload: { ediv, rand } });
