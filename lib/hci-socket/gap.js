...
class Gap {
  ...
  constructor(hci) {
    ...
    // });
    this.hciSubscription = hci.source.subscribe(({ event, payload }) => {
  ...
  handleEvent (event, payload) {
    switch (event) {
      case 'error': this.onHciError(payload);
        break;
      case 'leScanParametersSet': this.onHciLeScanParametersSet(payload);
        break;
      case 'leScanEnableSet': this.onHciLeScanEnableSet(payload);
        break;
      case 'leAdvertisingReport': this.onHciLeAdvertisingReport(payload);
        break;
      case 'leScanEnableSetCmd': this.onLeScanEnableSetCmd(payload);
    }
  }
...
// Called when receive an event "Command Complete" for "LE Set Scan Enable"
  onHciLeScanEnableSet({ status })  {
    ...
    if (this._scanState === 'starting') {
      ...
      // TODO test
      this.source.next({ event: 'scanStart', payload: { filterDuplicates: this._scanFilterDuplicates }});
      // this.emit('scanStart', this._scanFilterDuplicates);
    } else if (this._scanState === 'stopping') {
      this._scanState = 'stopped';

      // TODO test
      this.source.next({ event: 'scanStop' });
      // this.emit('scanStop');
    }
  }
...
// Called when we see the actual command "LE Set Scan Enable"
  ...
  onHciLeAdvertisingReport({ status, type, address, addressType, eir, rssi })  {
    ...
    const discoveryCount = (previouslyDiscovered ? this._discoveries[address].count : 0) + 1;
    ...
    if (type === 0x04 || !connectable || (discoveryCount > 1 && !hasScanResponse) || process.env.NOBLE_REPORT_ALL_HCI_EVENTS) {
      // TODO test
      this.source.next({ event: 'discover', payload: { status, address, addressType, connectable, advertisement, rssi } });
