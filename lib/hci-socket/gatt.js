...
class Gatt {
  constructor(address, aclStream) {
    ...
    this._valueHandleServices = {};
    this._valueHandleCharacteristics = {};
  ...
  onAclStreamData({ cid, data }) {
    ...
    if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
      ...
      const serviceUuid = this._valueHandleServices[valueHandle];
      const characteristicUuid = this._valueHandleCharacteristics[valueHandle];
      ...
      if (serviceUuid && characteristicUuid) {
        this.source.next({ event: 'notification', payload: { address: this._address, serviceUuid, characteristicUuid, valueHandle, valueData } });
      }
      // }
      // }
      // }
    ...
    const callback = (data) => {
      ...
      if (opcode !== ATT_OP_READ_BY_TYPE_RESP || characteristics[characteristics.length - 1].valueHandle === service.endHandle) {
        ...
        for (let i = 0, l = characteristics.length; i < l; i++) {
          ...
          const { properties, uuid, valueHandle } = chara;
          ...
          if (i !== 0) {
            characteristics[i - 1].endHandle = chara.startHandle - 1;
          }

          if (i === (characteristics.length - 1)) {
            chara.endHandle = service.endHandle;
          }

          this._characteristics[serviceUuid][uuid] = chara;
          this._valueHandleServices[valueHandle] = serviceUuid;
          this._valueHandleCharacteristics[valueHandle] = uuid;
