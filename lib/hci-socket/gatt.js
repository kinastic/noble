...
Gatt.prototype._queueCommand = function(buffer, callback, writeCallback) {
  debug(`command queue: length = ${this._commandQueue.length}, content = ` + JSON.stringify(this._commandQueue))
  ...
  if (this._currentCommand === null) {
    ...
    debug('_currentCommand is not NULL');
  }
};
...
Gatt.prototype.exchangeMtu = function(mtu) {
  debug('Enqueue mtuRequest');
  this._queueCommand(this.mtuRequest(mtu), function(data) {
    var opcode = data[0];
    
    debug(`mtuRequest opcode = ${opcode}`);
...
Gatt.prototype.discoverServices = function(uuids) {
  debug(`Going to discover Services uuids length = ${(uuids || []).length}`);
  var services = [];
  ...
  debug('enqueuing discoverServices');
...
Gatt.prototype.write = function(serviceUuid, characteristicUuid, data, withoutResponse) {
  ...
  if (withoutResponse) {
    debug('Enqueue writeWithoutResponse');
...
Gatt.prototype.readHandle = function(handle) {
  this._queueCommand(this.readRequest(handle), function(data) {
    var opcode = data[0];var debug = require('debug')('att');
