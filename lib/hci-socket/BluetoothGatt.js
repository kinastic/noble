...
class BluetoothGatt {
  constructor(hci, handle, address, addressType, userMode) {
    ...
    this.discoverServicePromise = null;
  ...
  discoverServices(uuids) {
    if (!this.discoverServicePromise) {
      this.discoverServicePromise = new Promise((resolve, reject) => {
        this.discoverServiceResolve = resolve;
        this.discoverServiceReject = reject;
        this.gatt.discoverServices(uuids || []);
      });
    }

    return this.discoverServicePromise;
  }
  ...
  onServicesDiscovered({ address, serviceUuids }) {
    if (this.address === address) {
      if (this.discoverServiceResolve) {
        // const services = serviceUuids.map((uuid) => new Service(this, this.address, uuid));
        this.discoverServiceResolve(serviceUuids);
      }
      this.discoverServiceReject = null;
      this.discoverServiceResolve = null;
      this.discoverServicePromise = null;
    }
  }

  onWrite({ address, serviceUuid, characteristicUuid }) {
    this.source.next({ event: 'write', payload: { address, serviceUuid, characteristicUuid } });
  }
  ...
  onDisconnComplete({ handle, reason }) {
    ...
    if (this.discoverServiceReject) {
      this.discoverServiceReject();
    }
